...
pheno2 = gather(pheno,"DateOfFirstFlowering.PosA","DateOfFirstFlowering.PosB",key="posAB",value="flowering")

# convert calender dates into days to flowering
mm = substr(pheno2$flowering,1,2)
mm[mm=="03"] = 0
mm[mm=="04"] = 31
dd = substr(pheno2$flowering,4,5)
pheno2$flowering = as.numeric(mm) + as.numeric(dd)
...
pheno2 = pheno2[-which(is.na(pheno2$flowering)),]
...
saveRDS(out,file="../output/NeiGWAS_Wuest_et_al_sim_all_floweringZ.rds")
